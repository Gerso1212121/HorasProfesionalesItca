...
> Recuerda: Establecer límites no es construir muros, es construir puertas con cerraduras a las que solo tú tienes la llave.
      ...
      for (final module in sampleModules) {
        await _db.createModulo(
          ...
          sincronizado: (module['sincronizado'] as int) == 1,
        );
      }
  ...
  /// Cargar datos de ejemplo para ejercicios
  static Future<void> loadSampleExercises() async {
    try {
      final now = DateTime.now();
      ...
      final sampleExercises = [
        {
          ...
          'objetivos': 'Reducir ansiedad, mejorar relajación',
          'instrucciones': 'Inhala por 4, mantén por 7, exhala por 8 segundos',
          'fecha_creacion': now.toIso8601String(),
        },
        {
          'titulo': 'Meditación Mindfulness',
          'descripcion': 'Práctica básica de atención plena',
          'categoria': 'Mindfulness',
          'tipo': 'meditacion',
          'duracion_minutos': 10,
          'dificultad': 'Principiante',
          'objetivos': 'Aumentar consciencia, reducir estrés',
          'instrucciones': 'Siéntate cómodamente y observa tu respiración',
          'fecha_creacion': now.toIso8601String(),
        },
        {
          'titulo': 'Visualización Positiva',
          'descripcion':
              'Ejercicio de visualización para mejorar el estado de ánimo',
          'categoria': 'Visualización',
          'tipo': 'visualization',
          'duracion_minutos': 15,
          'dificultad': 'Intermedio',
          'objetivos': 'Mejorar autoestima, reducir pensamientos negativos',
          'instrucciones':
              'Imagina un lugar seguro y experimenta sensaciones positivas',
          'fecha_creacion': now.toIso8601String(),
        },
      ];

      for (final exercise in sampleExercises) {
        await _db.createEjercicio(
          titulo: exercise['titulo'] as String,
          descripcion: exercise['descripcion'] as String,
          categoria: exercise['categoria'] as String,
          tipo: exercise['tipo'] as String,
          duracionMinutos: exercise['duracion_minutos'] as int,
          dificultad: exercise['dificultad'] as String,
          objetivos: exercise['objetivos'] as String,
          instrucciones: exercise['instrucciones'] as String,
        );
      }

      print('Ejercicios de ejemplo cargados exitosamente');
    } catch (e) {
      print('Error cargando ejercicios de ejemplo: $e');
    }
  }
